...
- **Nästa:** Erik verifierar via `npm run dev` (envelope-toggle på) — **tre klipp, inga regressioner**: (a) face-on → nu impact + impact cluster, envelope `[3.35→4.83]`; (b) DTL oavklippt → `[6.72→8.38]`, impact 7.78; (c) DTL avklippt → "uniform baseline · clipped tail" utan impact; läs `[START-DIAG]`-loggarna (`startSecChosen ≈ 6.8`, `spd`-rampen) → **ta bort TEMP-diagnostiken + `ADDRESS_DEPART_TOL`**; enhetstest på envelope-/settle-/impact-logiken; 20-klipp-utvärdering + D-3-cutover. Detaljer i [pose-detection.md](pose-detection.md).
