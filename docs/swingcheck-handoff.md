...
- **Nästa:** Erik verifierar start/impact på DTL-klippet via `npm run dev` (envelope-toggle på, läs `[START-DIAG]`-loggarna: `startSecChosen ≈ 6.8`, `speedOnsetSec`, per-frame `spd`-rampen) → **ta bort TEMP-diagnostiken + `ADDRESS_DEPART_TOL`**; enhetstest på envelope-/settle-/impact-logiken; utvärdera pose-impact mot manuell etikett på 20 klipp + D-3-cutover. Detaljer i [pose-detection.md](pose-detection.md).
