...
- **Pass 2-buggfix (2026-07-08):** verkligt 30fps-klipp visade `top`/`impact`/`ft` kollapsade till klippslutet (top 3.00 · impact 3.07 · ft 3.07) → fallbacken missade impact. Leadhypotes (verifieras via ny `debug`-logg): apex (min y) låste på follow-through-**finishen**, inte baksvingstoppen. Åtgärdat: (1) per-sampel `{t,y,vy,speed}`-trace i `PoseSelect`-loggen; (2) fallback sprider uniformt i tid över `[backswingStart, spanEnd]` (kastar fas-fönstren); (3) impact-gate på nedåtrörelse (`vy>0`) + minsta downswing-tid `MIN_DOWNSWING_SEC` 0.12 s. Bakom `VITE_DEV_PREVIEW`, `frameExtractor.ts` orörd, default even oförändrad.
- **Nästa:** fältbekräfta finish-vs-top-roten via `debug`-loggen; självhosta WASM (D-2 a) + SW-precache; enhetstest på fas-logiken; utvärdera pose-impact mot manuell etikett på 20 klipp (D-3). Detaljer i [pose-detection.md](pose-detection.md).
