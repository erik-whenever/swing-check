...
- **Nästa:** Eriks manuella checkpoint-2 (envelope-toggle på riktiga klipp, `PoseSelect`-logg: `envelopeSec`, `impactClusterApplied`, `clippedTail`); självhosta WASM (D-2 a) + SW-precache; enhetstest på envelope-/settle-/impact-logiken; utvärdera pose-impact mot manuell etikett på 20 klipp + D-3-cutover. Detaljer i [pose-detection.md](pose-detection.md).
