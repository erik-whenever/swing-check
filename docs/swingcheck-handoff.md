...
- **Nästa:** enhetstest på envelope-/settle-/impact-logiken (`poseEnvelope.ts`); Eriks manuella checkpoint-2 (envelope-toggle på riktiga klipp + offline-verifiering av WASM-precachen via `npm run dev`, `PoseSelect`-logg: `envelopeSec`, `impactClusterApplied`, `clippedTail`); utvärdera pose-impact mot manuell etikett på 20 klipp + D-3-cutover. Detaljer i [pose-detection.md](pose-detection.md).
